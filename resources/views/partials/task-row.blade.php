...
<tr class="clickable-row" id="{{ ($highlightId ?? null) == $task->id ? 'highlight-task' : 'task-row-' . $task->id }}"
    ...
    <td>
        ...
        <div class="dropdown">
            ...
            <button class="status-btn status-{{ $statusSlug }} dropdown-toggle" type="button"
                id="statusDropdown{{ $task->id }}" data-task-id="{{ $task->id }}" data-bs-toggle="dropdown"
                aria-expanded="false">
            ...
            <div class="dropdown-menu" aria-labelledby="statusDropdown{{ $task->id }}">
                @if(Auth::user()->role == 'admin')
                    @if($currentStatus == 'Hold' || $currentStatus == 'Delivered')
                        <a class="dropdown-item" href="#" data-status="Resume Progress"><i class="bi bi-play-circle"></i> Resume
                            Progress</a>
                    @endif
        ...
        @php
            ...
            if ($isFinished) {
                if ($task->status->name == 'Delivered') {
                    ...
                    $timeLeftString = \Carbon\Carbon::parse($dateObj)->format('d M Y');
    ...
    <td class="icon-cell">
        <div class="mockup-wrapper">
            ...
            @endforeach
            <img src="{{ $task->mockups->first() ? Storage::url($task->mockups->first()->file_path) : asset('assets/img/default.png') }}"
                class="mockup-display">
    ...
    <td>
        ...
        <button class="progress {{ $progressClass }}" type="button" style="cursor: default; pointer-events: none;">
