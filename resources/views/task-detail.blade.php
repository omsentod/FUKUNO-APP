  ...
  <div class="page" data-user-name="{{ Auth::user()->name }}">
    <div class="detail-task-container">
      ...
      <div class="task-header">
        @php
          // Cek jika 'from' adalah full URL (dari task row link)
          if (request('from') && filter_var(request('from'), FILTER_VALIDATE_URL)) {
            $backUrl = request('from'); // Gunakan full URL untuk preserve state
          } elseif (request('from') == 'archive') {
            $backUrl = route('archive');
          } elseif (request('from') == 'trash') {
            $backUrl = route('trash');
          } else {
            $backUrl = route('task'); // Default
          }
        @endphp
        ...
        <div class="task-detail-table">
          <div class="left-section">
            <table>
              <tr>
                <th>No. PO</th>
                <td>{{ $mainTask->no_invoice }}</td>
                <th>Tanggal Mulai</th>
                <td>{{ $mainTask->created_at->format('j M Y') }}</td>
              </tr>
              <tr>
                <th>Klien</th>
                <td>{{ $mainTask->nama_pelanggan }}</td>
            ...
            <div class="spec-note">
              <div class="spec">
                ...
                <table>
                  <tr>
                    <th>Warna</th>
                    <td>{{ $mainTask->warna ?? '-' }}</td>
                  </tr>
                  <tr>
                    <th>Article / Model</th>
                    <td colspan="3">{{ $mainTask->model ?? '-' }}</td>
                  </tr>
                  <tr>
                    <th>Material</th>
                    <td colspan="3">{{ $mainTask->bahan ?? '-' }}</td>
                  </tr>
                </table>
              </div>
              <div class="note">
          ...
          <div class="right-section">
            ...
            <div class="image-gallery" id="image-gallery">
              @forelse($mainTask->mockups as $mockup) <img src="{{ Storage::url($mockup->file_path) }}" alt="Mockup"
                class="gallery-image">
              @empty
        ...
        <div class="bahan-section mb-4 p-3 border rounded bg-light">
          ...
          <div class="row">
            <div class="col-md-6">
              ...
              <textarea class="form-control bahan-input" id="bahan-terpakai" data-task-id="{{ $mainTask->id }}"
            ...
            <div class="col-md-6">
              ...
              <textarea class="form-control bahan-input" id="bahan-reject" data-task-id="{{ $mainTask->id }}"
                ...
                <div class="dropdown-content" style="display: block;"> @forelse($line->checklists as $checklist)
                  ...
                  <label>
                    <input type="checkbox"
                      ...
                      data-id="{{ $checklist->id }}" {{ $checklist->is_completed ? 'checked' : '' }}>
          ...
          <div class="chat-container" id="chat-container" data-task-id="{{ $mainTask->id }}">
            @forelse($comments as $comment)
              <div id="comment-{{ $comment->id }}"
                class="comment-bubble {{ $comment->user_id == Auth::id() ? 'own' : '' }}">
