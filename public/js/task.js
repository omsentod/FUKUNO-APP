...
`;
    ...
    if (savedClickedId) {
        ...
        setTimeout(() => {
            ...
            if (targetRow) {
                ...
                // 2. Beri efek highlight (Pakai animasi yg sudah ada: #highlight-task)
                // Karena ID harus unik, pastikan tidak ada elemen lain yg pakai ID ini
                ...
                // 3. Hapus ID setelah animasi selesai (sesuai durasi di CSS: 8.0s)
                setTimeout(() => {
                    if (targetRow.id === 'highlight-task') {
                        targetRow.removeAttribute('id');
                    }
                    sessionStorage.removeItem("clickedTaskId");
                }, 8000); // 8 detik sesuai CSS highlight-flash
            } else if (savedScrollPos) {
                // Fallback: Jika baris tidak ketemu (misal pagination), pakai posisi scroll lama
                window.scrollTo(0, parseInt(savedScrollPos));
            }
        }, 300);
    } else if (savedScrollPos) {
        // Jika tidak ada ID yg diklik, kembalikan scroll saja
        setTimeout(() => {
            window.scrollTo(0, parseInt(savedScrollPos));
        }, 100);
    }
    ...
    document.body.addEventListener("click", (e) => {
        ...
        if (row) {
            sessionStorage.setItem("taskScrollPos", window.scrollY);

            // Ambil ID dari URL di data-url (karena tidak ada data-id di tr)
            // Format: .../task/detail/{id}?from=...
            const url = row.dataset.url;
            if (url) {
