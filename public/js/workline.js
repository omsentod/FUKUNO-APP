...
document.addEventListener("DOMContentLoaded", () => {
    ...
    addBtn.addEventListener("click", () => {
      showPopup("Tambah Line Pekerjaan", "", (nama) => {
        ...
        showToast("Line pekerjaan berhasil ditambahkan!");
  ...
  function showPopup(title, currentName, onSubmit) {
    ...
    popup.innerHTML = `
      <h5>${title}</h5>
      <input type="text" id="namaLine" placeholder="Nama line pekerjaan" value="${currentName}">
  ...
  function attachActions(row) {
    ...
    editBtn.addEventListener("click", () => {
      ...
      showPopup("Edit Line Pekerjaan", namaSekarang, (namaBaru) => {
        ...
        showToast("Nama line pekerjaan berhasil diperbarui!");
      });
    });
    ...
    deleteBtn.addEventListener("click", () => {
      if (confirm("Apakah kamu yakin ingin menghapus line pekerjaan ini?")) {
        row.remove();
        showToast("Line pekerjaan berhasil dihapus!");
      }
    });
  }
