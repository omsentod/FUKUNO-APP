        ...
        toggleNotifBtn.addEventListener('click', function (e) {
            ...
            fetch('/user/toggle-notification', {
                ...
                .then(result => {
                    if (result.success) {
                        // Update icon
                        const icon = toggleNotifBtn.querySelector('i');
                        if (result.notification_enabled) {
                            icon.classList.remove('bi-bell-slash-fill');
                            icon.classList.add('bi-bell-fill');
                            toggleNotifBtn.title = 'Matikan notifikasi';
                        } else {
                            icon.classList.remove('bi-bell-fill');
                            icon.classList.add('bi-bell-slash-fill');
                            toggleNotifBtn.title = 'Aktifkan notifikasi';
                        }
                        console.log(result.message);
                    }
                })
                .catch(error => console.error('Gagal toggle notifikasi:', error));
        });
    }
